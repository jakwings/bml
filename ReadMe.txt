...
{::: Such comment must be enclosed within the same amount of colons. :::}

{::
  Want nested comment like {: this :} to easily comment out some content?
...
{article
  ...
  {comment:
    Metadata/Attributes/Arguments and content are separated by a colon \'U+3A'.
  \comment}

  {banner {id:banner} : {utils/banner \"is beautiful?"} }

  {title: Alternative Markup Language}

  {description: {{ Composing rich-text documents with {ease}. }} }

  {more:

    Normally, curly brackets \{\} in text need to be escaped with backslash \\.
